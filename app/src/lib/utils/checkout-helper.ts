...
export class CheckoutHelper {
  ...
  static createTenderItems(type: string, total: number) {
    return [{
      type,
      itemNo: 'a123',
      value: parseFloat(total.toFixed(2)),
  ...
  static createLineItems(
    ...
    const filteredCartItems = cartItems.filter(
      (item: any) => 
        item.category !== this.SHIPPING_CATEGORY && 
        item.category !== this.TAX_CATEGORY
    );
    ...
    const hemmingItems = [cartItems.find((item: any) => item.category === this.HEMMING)]
      .filter(Boolean)
      .map(() => ({
